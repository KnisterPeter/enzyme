import { REACT_ROOTS } from './ReactDevToolsHook';
...
/**
 * Returns all mounted React Root-Components.
 * 
 * @returns {InjectWrapper[]}
 */
export function getRoots() {
  // Map roots here into InjectWrappers to get around cyclic dependency
  return Object.keys(REACT_ROOTS).map(rootId => new InjectWrapper(REACT_ROOTS[rootId]));
}
 ...
 * @class InjectWrapper
 */
export default class InjectWrapper {
  ...
  ref(refname) {
    if (this.root !== this) {
      throw new Error('InjectWrapper::ref(refname) can only be called on the root');
    }
  ...
  update() {
    if (this.root !== this) {
      // TODO(lmr): this requirement may not be necessary for the InjectWrapper
      throw new Error('InjectWrapper::update() can only be called on the root');
    }
    this.single(() => {
  ...
  setProps(props) {
    if (this.root !== this) {
      throw new Error('InjectWrapper::setProps() can only be called on the root');
    }
  ...
  setState(state) {
    if (this.root !== this) {
      throw new Error('InjectWrapper::setState() can only be called on the root');
    }
  ...
  setContext(context) {
    if (this.root !== this) {
      throw new Error('InjectWrapper::setContext() can only be called on the root');
    }
  ...
  simulate(event, ...args) {
    this.single(n => {
      ...
      if (!eventFn) {
        throw new TypeError(`InjectWrapper::simulate() event '${event}' does not exist`);
      }
  ...
  state(name) {
    if (this.root !== this) {
      throw new Error('InjectWrapper::state() can only be called on the root');
    }
  ...
  hasClass(className) {
    if (className && className.indexOf('.') !== -1) {
      console.log(
        'It looks like you\'re calling `InjectWrapper::hasClass()` with a CSS selector. ' +
  ...
  /**
   * Maps the current array of nodes to another array. Each node is passed in as a `InjectWrapper`
   * to the map function.
   *
   * @param {Function} fn
   * @returns {Array}
   ...
   *
   * @param {InjectWrapper|ReactElement|Array<ReactElement>} node
   * @returns {InjectWrapper}
   */
  wrap(node) {
    if (node instanceof InjectWrapper) {
      return node;
    }
    return new InjectWrapper(node, this.root);
  }
}
